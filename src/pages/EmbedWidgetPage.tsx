...
export default function EmbedWidgetPage() {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        window.parent.postMessage({ type: "vouchy-resize", height: height + 50 }, "*");
      }
    });
  ...
  if (loading) return (
    <div 
      ...
      style={{ backgroundColor: config.darkMode ? "#0a0a0a" : "transparent" }}
    >
...
function CarouselView({ testimonials, config, carouselVisible, navStyle, navIconColor, navBgColor, autoPlay, autoPlaySpeed }: {
  ...
  return (
    <div>
      <div className="grid gap-4 px-4" style={{ gridTemplateColumns: `repeat(${carouselVisible}, minmax(0, 1fr))` }}>
